...
if __name__ == '__main__':
    ...
    with Tee(log_filepath, mode='w'):
        ...
        train_dataset, val_dataset = (dataset, list(dataset)) if train_size >= len(dataset) else (dataset[:train_size], dataset[train_size:])
        ...
        # Variables to store the last sample for final plot
        ...
        if val_dataset:
            ...
            for i, data_sample in enumerate(val_dataset):
                ...
                # Regenerate mesh for the validation sample
                ...
                # Get original coordinates and boundary nodes
            ...
            # (predVStrue, time_comparison, etc. plots remain here)
            # ...

        # --- MODIFIED: Final Comparison Plot ---
        ...
        if last_sample_data_for_plot and last_sample_data_for_plot.get("gat_mesh"):
            ...
            l2_i = last_sample_data_for_plot["l2_initial"]
            plt.sca(axes[0]); dolfin.plot(initial_mesh)
            axes[0].set_title(f"Initial Mesh\nL2 Err: {l2_i:.2e}" if l2_i >= 0 else "Initial Mesh")
            
            # 2. Classical Adapted Mesh
            classical_mesh = last_sample_data_for_plot["classical_mesh"]
            l2_c = last_sample_data_for_plot["l2_classical"]
            plt.sca(axes[1]); dolfin.plot(classical_mesh)
            axes[1].set_title(f"Classical Adapted Mesh\nL2 Err: {l2_c:.2e}" if l2_c >= 0 else "Classical Adapted (Solve Failed)")

            # 3. GAT Adapted Mesh
            gat_mesh = last_sample_data_for_plot["gat_mesh"]
            l2_g = last_sample_data_for_plot["l2_gat"]
            plt.sca(axes[2]); dolfin.plot(gat_mesh)
            axes[2].set_title(f"GAT Adapted Mesh\nL2 Err: {l2_g:.2e}" if l2_g >= 0 else "GAT Adapted (Solve Failed)")

            for ax in axes:
                if config.MESH_TYPE == 'pipe':
                    ax.set_aspect('equal')
            ...
            print("Skipping final comparison plot as last validation sample was not fully processed.")
