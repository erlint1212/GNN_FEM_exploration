...
from models.GraphSAGE import RAdaptGraphSAGE
...
if __name__ == '__main__':
    ...
    with Tee(log_filepath, mode='w'): # Start logging
        ...
        if not dataset:
            ...
            sys.exit(1) # Use sys.exit for a clean exit with error code
        ...
        if train_size >= len(dataset):
            ...
            val_dataset = list(dataset) # Use a copy for validation
        else:
        ...
        if dataset and hasattr(dataset[0], 'x') and dataset[0].x is not None:
            ...
            print("Error: Dataset is empty or first sample has no features 'x'. Cannot determine input feature dimension. Exiting.")
            sys.exit(1)
        ...
        # (Optional: Print avg times and errors from dataset generation if desired)
        ...
        for epoch in range(config.EPOCHS):
            ...
            for batch_idx, batch_data in enumerate(train_loader): # Use batch_data to avoid conflict
                batch_data = batch_data.to(config.DEVICE) # Ensure batch is on device
                optimizer.zero_grad()
        ...
        if val_dataset:
            ...
            # (PredVsTrue plotting - same as before)
            ...
            for i, data_sample in enumerate(val_dataset):
                ...
                except: print("Could not eval original_coords_str from data_sample"); original_coords_val_np = None
                
                if current_mesh_type == 'square': # Regenerate mesh
                    nx_val = current_mesh_dims.get('nx'); ny_val = current_mesh_dims.get('ny')
                    if nx_val is None or ny_val is None: print(f"    nx/ny not found for val sample {i+1}. Skip."); continue
                ...
                if original_coords_val_np is None or original_coords_val_np.shape[0] != initial_mesh_val.num_vertices():
                    original_coords_val_np = initial_mesh_val.coordinates() # Fallback
            ...
            # (Reporting and Plotting validation results - same as before, using config variables)
            # ... (Ensure correct list names for plotting, e.g., val_gat_inference_times_list_bench) ...
            # ... (Make sure to use config.MODEL_NAME etc. for titles and filenames in plot_funcs calls) ...

        # Example Inference
        print(f"\n--- Example Inference & Plot ({config.MESH_TYPE} Geometry) ---")
        # ... (Ensure this section also uses the robust boundary fixing and quality checks) ...
        # ... (And uses config variables where appropriate) ...
