...
if __name__ == '__main__':
    ...
    with Tee(log_filepath, mode='w'):
        ...
        # This call generates the first set of plots for Sample 1
        ...
        if val_dataset:
            ...
            for i, data_sample in enumerate(val_dataset):
                # (This loop remains the same as the previous version, calculating all metrics
                # and populating `last_sample_data_for_plot` on the final iteration)
            ...
            plot_funcs.predVStrue(label_val_true=val_true_coords_list, label_val_pred=val_pred_coords_list, label_train_true=train_true_coords_history, label_train_pred=train_pred_coords_history, model_name=config.MODEL_NAME, output=SESSION_OUTPUT_DIR, show=False)
            plot_funcs.plot_time_comparison(classical_times=val_classical_times_list_bench, gat_times=val_gat_inference_times_list_bench, title=f"Mesh Adapt Time ({config.MODEL_NAME} vs Classical)", use_box_plot=True, output=SESSION_OUTPUT_DIR, show=False)
            ...
            if valid_indices:
                ...
                plot_funcs.plot_accuracy_vs_cost(classical_costs=plot_dofs, classical_accuracies=plot_classical_errors, gat_costs=plot_dofs, gat_accuracies=plot_gat_errors, title=f"Accuracy vs. Cost ({config.MODEL_NAME})", output=SESSION_OUTPUT_DIR, show=False)
                plot_funcs.plot_convergence(classical_dofs=plot_dofs, classical_errors=plot_classical_errors, gat_dofs=plot_dofs, gat_errors=plot_gat_errors, title=f"Convergence Plot ({config.MODEL_NAME})", output=SESSION_OUTPUT_DIR, show=False)

            # --- Generate and Save Benchmark Summary JSON ---
            print("\n--- Generating Benchmark Summary JSON ---")
            summary_data = { "model_name": config.MODEL_NAME, "session_timestamp": session_timestamp, "device": str(config.DEVICE), "parameters": { "epochs": config.EPOCHS, "num_validation_samples_benchmarked": len(val_dataset) if val_dataset else 0, }, "classical_r_adaptivity_times_seconds": { "mean": np.mean(val_classical_times_list_bench).item() if val_classical_times_list_bench else None, }, "gat_inference_times_seconds": { "mean": np.mean(val_gat_inference_times_list_bench).item() if val_gat_inference_times_list_bench else None, } }
        ...
        if last_sample_data_for_plot and last_sample_data_for_plot.get("gat_mesh"):
            ...
            # Generate the final 1x3 mesh comparison plot
            ...
            # Generate density plots of the coordinates
